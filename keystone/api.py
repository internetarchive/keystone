...
###############################################################################
# Temporary ARCH Proxy Login Endpoints to support legacy ARCH during post-split
...
class ArchUserResponse(Schema):
    """UserSchema extended with a fullname property."""
...
@private_api.post("/proxy_login", response={200: ArchUserResponse, codes_4xx: None})
def private_proxy_login(request, payload: ProxyLoginRequest):
    ...
    if not validate_password(payload.password, user.password):
        return 403, None
    return 200, model_to_dict(user) | {"fullname": user.get_full_name()}


@private_api.post("/proxy_change_password", response={200: None, codes_4xx: None})
def private_proxy_change_password(request, payload: ProxyChangePasswordRequest):
...
@private_api.get("/user", response={200: ArchUserResponse, codes_4xx: None})
def private_get_user(request, username: str):
    """Retrieve a user."""
